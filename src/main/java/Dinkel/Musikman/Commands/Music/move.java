...
public class move extends Command{
	...
	public void commandCode(MessageReceivedEvent eventMessage, List<String> args, boolean publicExec) {
		...
		if(args.size() < 2) {
			this.publicExec(publicExec, () -> {channel.sendMessage("not enought arguments. See `!help`").queue();});
			return;
		}
		...
		if(helper.isInteger(arg1) && helper.isInteger(arg2)) {
			...
			this.publicExec(publicExec, () -> {channel.sendMessage("moved track `" + arg1 + " to " + arg2 +"`").queue();});
			return;
		}else {
